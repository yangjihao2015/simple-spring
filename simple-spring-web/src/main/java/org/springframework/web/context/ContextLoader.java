 ...
 *
 * 处理contextConfigLocation上下文参数，　并将其值传递给上下文实例，
 * 将其解析成为可能有多个逗号和空格分割的文件路径．
 * 比如：　WEB-INF/applicationContext1.xml, WEB-INF/applicationContext2.xml
 * 也支持蚂蚁风格路径模式，　
 * 比如：　WEB-INF/*Context.xml,WEB-INF/spring*.xml　或者　WEB-INF/&#42;&#42;/*Context.xml
 * 如果没有明确指定，　上下文实现应该使用默认位置（/WEB-INF/applicationContext.xml）
 *
 * 注意: 在多个配置位置的情况下，　后面的bean定义将覆盖之前的加载的文件中的bean定义
 * 这个特性可被用来通过额外的XML文件故意重写某些bean定义．
 *
 * 除了加载根ApplicationContext, 此类可以选择地加载或获取并共享父ApplicationContext挂接到根ApplicationContext.
 * 请参阅#loadParentContext(ServletContext)
 *
 * 从spring 3.1起，ContextLoader支持通过#ContextLoader(WebApplicationContext)构造函数注入根ApplicationContext,
 * 允许在Servlet 3.0以上环境中进行编程配置，　有关用法示例，　请参见org.springframework.web.WebApplicationInitializer
 *
